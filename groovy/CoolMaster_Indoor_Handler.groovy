...
def installed() {
    log.trace "Executing 'installed' ${device.label}"
...
{	
    ...
    log.debug "setTemperatureParams NEW setpoint is ${setpoint} rt is ${temperature}"
}

/* Parse device response */
def parseData(data) {
    log.trace "Executing 'parseData'"
...
/* capability Switch */
def on(){
    log.debug "ON: last op state: ${state.lastOperatingState}"
...
def off(){
    ...
    log.debug "OFF: last op state: ${state.lastOperatingState}"
...
def emergencyHeat(){
	log.debug "EmergencyHeat isn't emplemented yet"
	setMode("heat",null)
}
...
private switchTempLimits(tempUnit){
    ...
    log.debug "switchTempLimits ${state.limits}"
}
...
log.debug "setOperatingState ${setpoint}"
